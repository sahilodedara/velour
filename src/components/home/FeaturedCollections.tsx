      ...
      <Container>
        <Reveal>
          <SectionHeading
            ...
            title={<>Curated by category,<br /> chosen for a lifetime</>}
            description="From sculptural leather goods to horological craft — explore the maisons defining modern luxury."
          />
        </Reveal>
        ...
        <Stagger className="mt-12 grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-4">
          {cats.map((c, i) => (
            <StaggerItem key={c.slug} className={i === 0 ? "col-span-2 md:row-span-1" : ""}>
              <CollectionCard slug={c.slug} name={c.name} tagline={c.tagline ?? ""} palette={c.palette} wide={i === 0} />
            </StaggerItem>
          ))}
          <StaggerItem>
            <Link
            ...
            >
              <span className="font-display text-2xl">View All</span>
              <span className="flex items-center gap-1 text-xs uppercase tracking-[0.2em] text-ink-muted transition-colors group-hover:text-gold-deep">
                The full edit <ArrowUpRight size={13} />
              </span>
            </Link>
          </StaggerItem>
        </Stagger>
  ...
  return (
    <Link href={`/shop?category=${slug}`} className="group relative block h-64 overflow-hidden md:h-80">
      <div className="absolute inset-0 transition-transform duration-[900ms] ease-[var(--ease-luxe)] group-hover:scale-105">
        <ProductArtwork palette={palette} monogram={name[0]} category={slug} name={name} variant={wide ? "hero" : "card"} />
      </div>
