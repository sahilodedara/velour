  ...
  return (
    <article className="group relative">
      <div className="relative aspect-[4/5] overflow-hidden bg-bg-sunken">
        <Link href={`/product/${product.slug}`} aria-label={product.name} className="block h-full w-full">
          ...
          <div className="absolute inset-0 transition-opacity duration-700 ease-[var(--ease-luxe)] group-hover:opacity-0">
            <ProductArtwork palette={product.palette} monogram={monogram} category={product.category} name={product.name} />
          </div>
        ...
        <div className="pointer-events-none absolute left-3 top-3 flex flex-col gap-2">
          ...
          {product.bestSeller && !product.newArrival && <Badge tone="gold">Bestseller</Badge>}
        </div>
        ...
        <div className="absolute inset-x-0 bottom-0 flex translate-y-full gap-px bg-line opacity-0 transition-all duration-500 ease-[var(--ease-luxe)] group-hover:translate-y-0 group-hover:opacity-100">
          ...
          >
            <ShoppingBag size={14} /> {needsChoice ? "Select" : "Add"}
          </button>
        </div>
      </div>
      ...
      <Link href={`/product/${product.slug}`} className="mt-4 block">
        ...
        <h3 className="mt-1 text-sm leading-snug transition-colors group-hover:text-gold-deep">{product.name}</h3>
